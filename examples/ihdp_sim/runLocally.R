    ...
    for (k in seq_along(covariates)) {
      for (l in seq_along(settings)) {
        for (m in seq_along(p.score)) {
          existingResults <-
            getResultIntervals(methods[i], overlap[j], covariates[k], settings[l], p.score[m])
          
          resultsIntervals <-
            intervalSubtraction(baseResultInterval, existingResults)
          if (nrow(resultsIntervals) == 0)
            next
          
          Sys.setenv(METHOD = methods[i])
          Sys.setenv(OVERLAP = if (overlap[j])
            "true"
            else
              "false")
